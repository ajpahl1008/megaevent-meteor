	...
	Meteor.startup(function() {
		...
		if (MegaEvents.find().count() === 0) {
			...
			for (var i = 0; i < activeEvents.length; i++) {
				MegaEvents.insert({name: activeEvents[i], startDate:"12-12-1201", startTime:"12:00:00.000",  state:"active", complete:10, active:10, unstarted:12});
			}
			console.log("Adding Simulated Events - Inactive");
			for (var i = 0; i < inactiveEvents.length; i++) {
				MegaEvents.insert({name: inactiveEvents[i], startDate:"12-12-1023", startTime:"12:00:00.000", state:"inactive",complete:0, active:0, unstarted:12});
			}
			console.log("Adding Simulated Events - Complete");
			for (var i = 0; i < completeEvents.length; i++) {
				MegaEvents.insert({name: completeEvents[i], startDate:"12-12-1023", startTime:"12:00:00.000",  state:"complete",complete:100, active:0, unstarted:0});
			}
			console.log("Adding Simulated Events - Planning");
			for (var i = 0; i < planningEvents.length; i++) {
				MegaEvents.insert({name: planningEvents[i], startDate:"12-12-1023", startTime:"12:00:00.000",  state:"planning",complete:0, active:0, unstarted:100});
			}
		...
		if (MegaParticipants.find().count() === 0) {
			...
			for (var i = 0; i < sysadminParticipants.length; i++) {
				MegaParticipants.insert({name: sysadminParticipants[i], role:"sysadmin", phone:"555-1212", email:"dude@company.com"});
			}
			...
			for (var i = 0; i < ownersParticipants.length; i++) {
				MegaParticipants.insert({name: ownersParticipants[i], role:"owners", phone:"555-1212", email:"dude@company.com"});
			}
		}
